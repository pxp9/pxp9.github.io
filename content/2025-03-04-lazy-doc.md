...
models ? I told him that current the state of the AI is not that good to
suggesting code, but it was good to suggest other things like explanations, suggest
...
yet. That is how Lazy Doc idea came from.
...
documentation and if you do not want (because it is a massive amount of work) to
...
functions named the same different from each other just because their arity
...
auxiliar function does not make sense by itself and the entry point function
