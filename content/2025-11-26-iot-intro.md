...
date = "2025-11-26"
description = "An introduction step by step "
draft = false
[taxonomies]
tags = ["elixir"]
[extra]
...
If you are reading this article, it is because you are either interested in learning embedded systems, or how
to program your first firmware in a board in Elixir.
...
Peripherals or Sensors are external to MPUs, therefore, you will need to implement a driver
...
datasheet of the peripheral.

Every good MPU, MCU, sensor, or peripheral has an open datasheet. I would say, do
not buy a peripheral if you cannot find easily its datasheet online.

### Main differences between MPU and MCU.

- **Performance**, MPUs are much faster than MCUs. MPUs clock for a embedded
  device can run at 2.4Ghz compared to the MCUs, which can go between 800Mhz
  (STM32V8), 240 MHz (ESP32-S3) or 16 MHz (slowest Arduino).
...
- **Power consumption**. MCUs consume way less energy than MPUs, which makes them a
  perfect fit for field applications.
- **Applications**. MPUs can run AI models, Complex web applications, any kind of heavy
...
If you are new experimenting with this stuff, you might have skill issues like me
and you might want help from smarter or with more experienced people than you.
...
The reason they need to implement the Erlang VM is because are more hardware
...
- Supported Networking in their standard library [AtomVM
  ...
  *CAVEATS*: AtomVM is a quite new project and the devices might have partially
